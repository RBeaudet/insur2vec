...
class LAgefiScraping:

    def __init__(self, keywords: List[Tuple]) -> None:
        """Scrap articles from l'Agefi. Set a number of keywords as well
    ...
    def get_all_links(self, keyword: str, nb_pages: int) -> List[str]:
        ...
        for i in range(0, nb_pages + 1):
            url = f'https://www.agefi.fr/recherche?&keys={keyword}&page={i}'
            try :
                ...
                break
        with open('links/set_url_l_agefi.json', 'w') as f:
    ...
    def get_links(soup: BeautifulSoup) -> List[str]:
        ...
        """
        list_href = []
        sections = soup.find_all('article', class_='snippet--type-article_quotidien')
        for section in sections :
            if section.find('span', class_='snippet_payant') is None:
                list_href.append('https://www.agefi.fr' + section.find('a', class_='snippet_link').get('href'))
        return list_href
    ...
    def init_links_memory() -> List[str]:
        if not os.path.exists('links'):
            os.mkdir('links')
        if not os.path.exists('links/set_url_l_agefi.json'):
            with open('links/set_url_l_agefi.json', 'w') as f:
                scraped_url=[]
                json.dump(scraped_url, f)
            f.close()
        else :
            with open('links/set_url_l_agefi.json', 'r') as f:
